  ...
  return (
    <nav aria-label="Ödeme bölümleri" className="no-scrollbar overflow-x-auto">
      <ol className="flex min-w-0 items-center gap-2 sm:gap-3">
        {items.map((it, i) => (
          <li key={it.id} className="flex items-center gap-2 sm:gap-3 shrink-0">
            <button
              type="button"
              onClick={() => jump(it.id)}
...
function CheckoutPaymentTrustPanel({ paymentProvider }: { paymentProvider: Provider["id"] }) {
